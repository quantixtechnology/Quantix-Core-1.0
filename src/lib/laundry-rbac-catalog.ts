...
export const SYSTEM_ROLES: SystemRoleDef[] = [
  ...
  { code: "STORE_SUPERVISOR", name: "Store Supervisor", description: "Supervises store-floor operations across the counter.", perms: () => [DASH, ...moduleAll("store_ops"), ...screenAll("laundry", "orders").filter((k) => /\.(view|create|edit|print)$/.test(k)), ...screenAll("laundry", "customers").filter((k) => /\.(view|create|edit)$/.test(k)), ...screenAll("laundry", "subscriptions").filter((k) => k.endsWith(".view")), ...screenAll("laundry", "reports").filter((k) => k.endsWith(".view")), ...screenAll("laundry", "bags").filter((k) => k.endsWith(".view"))] },
