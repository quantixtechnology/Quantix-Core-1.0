          ...
          <Card>
            ...
            <CardContent className="pt-0">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {posProducts.map(p => (
                  <button key={p.id} onClick={() => addToCart(p)} className="p-3 rounded-lg border border-slate-200 hover:border-emerald-300 hover:bg-emerald-50 transition-colors duration-150 text-left">
                ...
                <div className="space-y-2">
                  {cart.map(item => (
                    <div key={item.id} className="flex items-center justify-between text-xs">
                      ...
                      <button onClick={() => setCart(prev => prev.filter(c => c.id !== item.id))} className="ml-2 text-red-400 hover:text-red-600 transition-colors duration-150"><Trash2 className="h-3 w-3" /></button>
                    </div>
                  ))}
                </div>
              )}
