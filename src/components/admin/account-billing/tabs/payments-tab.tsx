...
export function PaymentsTab({ onViewAccount }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch(`/api/admin/account-billing/payments?${p}`, { headers: getAuthHeaders() })
              ...
              : rows.map(r => (
                  <TableRow key={r.id} className="text-xs hover:bg-muted/30">
                    ...
                    <TableCell className="font-mono text-[10px]">
                      {r.invoiceNumber}
                      {r.billingPeriod && <span className="text-muted-foreground ml-1">({r.billingPeriod})</span>}
                    </TableCell>
                    <TableCell className="text-right tabular-nums font-semibold">{fmt(r.amount)}</TableCell>
                    <TableCell>{r.paymentMode ?? "—"}</TableCell>
                    <TableCell className="font-mono text-[10px]">{r.transactionId ?? "—"}</TableCell>
                    <TableCell>
                      <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${STATUS_CLS[r.status] ?? "bg-muted text-muted-foreground"}`}>
                        {r.status === "PENDING_VERIFICATION" ? "Pending Verif." : r.status}
                      </span>
                    </TableCell>
