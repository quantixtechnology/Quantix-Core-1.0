...
function CommissionHistory({ isAdmin }: { isAdmin: boolean }) {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      const qs = isAdmin ? `page=${page}&limit=15` : `page=${page}&limit=15&salesPersonId=${user?.id ?? ""}`
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="mt-4 flex-1 min-h-0">
        <Tabs defaultValue="calculator" className="flex flex-col h-full gap-4">
          <TabsList className="h-auto p-1 bg-muted/50 w-auto inline-flex shrink-0">
            <TabsTrigger value="calculator" className="gap-1.5 text-xs sm:text-sm">
              <BarChart3 className="h-3.5 w-3.5" /> New Signup Commission
            </TabsTrigger>
            <TabsTrigger value="history" className="gap-1.5 text-xs sm:text-sm">
              <Clock className="h-3.5 w-3.5" /> History
            </TabsTrigger>
            <TabsTrigger value="renewal" disabled className="gap-1.5 text-xs sm:text-sm">
              <TrendingUp className="h-3.5 w-3.5" />
              Renewal Commission
              <Badge variant="outline" className="ml-1 text-[9px] py-0 px-1.5 bg-amber-50 text-amber-700 border-amber-200">
                Soon
              </Badge>
            </TabsTrigger>
          </TabsList>
