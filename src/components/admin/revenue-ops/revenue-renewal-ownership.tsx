...
import { Search, Pencil, RefreshCcw, History } from "lucide-react"
...
export function RevenueRenewalOwnershipView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (auJson.success) setAudit(auJson.data.filter((e: AuditEntry) => e.assignmentType === "RENEWAL"))
    ...
    try {
      const res = await fetch("/api/admin/hrms/ownership", {
        ...
        body: JSON.stringify({
          ...
          renewalOwnerId: ownerId || undefined,
    ...
    <div className="p-6 space-y-5">
      <div>
        <div className="flex items-center gap-2 mb-0.5">
          <h1 className="text-2xl font-bold tracking-tight">Renewal Ownership</h1>
          <span className="text-xs bg-primary/10 text-primary font-semibold px-2 py-0.5 rounded-full">Quantix Internal</span>
        </div>
        <p className="text-sm text-muted-foreground">Assign Quantix employees as renewal owners for client businesses.</p>
      </div>

      <Tabs defaultValue="assignments">
        <TabsList>
          <TabsTrigger value="assignments" className="gap-1.5"><RefreshCcw className="h-3.5 w-3.5" />Assignments</TabsTrigger>
                      ...
                      {filtered.map((a) => (
                        <TableRow key={a.id}>
                          ...
                          <TableCell>
                            {a.renewalOwner ? (
                              <div>
                                <div className="text-sm font-medium">{a.renewalOwner.name}</div>
                                <div className="text-xs text-muted-foreground">{a.renewalOwner.designation}</div>
                              </div>
        ...
        <TabsContent value="audit" className="mt-4">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">Renewal Ownership Change History</CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {audit.length === 0 ? (
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Edit Renewal Owner — {editItem?.clientBusiness?.name}</DialogTitle>
          </DialogHeader>
