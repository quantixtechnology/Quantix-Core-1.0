...
import { Search, Pencil, Puzzle, History } from "lucide-react"
...
export function RevenueAddonOwnershipView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (auJson.success) setAudit(auJson.data.filter((e: AuditEntry) => e.assignmentType === "ADDON"))
    ...
    try {
      const res = await fetch("/api/admin/hrms/ownership", {
        ...
        body: JSON.stringify({
          ...
          signupOwnerId: editItem.signupOwnerId || undefined,
          renewalOwnerId: editItem.renewalOwnerId || undefined,
          addonOwnerId: ownerId || undefined,
          notes,
        }),
      })
      const json = await res.json()
      if (!json.success) throw new Error(json.error)
      toast.success("Add-On owner updated")
      setFormOpen(false)
      load()
    } catch (e) {
    ...
    <div className="p-6 space-y-5">
      <div>
        <div className="flex items-center gap-2 mb-0.5">
          <h1 className="text-2xl font-bold tracking-tight">Add-On Ownership</h1>
          <span className="text-xs bg-primary/10 text-primary font-semibold px-2 py-0.5 rounded-full">Quantix Internal</span>
        </div>
        <p className="text-sm text-muted-foreground">Assign Quantix employees as add-on owners for client businesses.</p>
      </div>

      <Tabs defaultValue="assignments">
        <TabsList>
          <TabsTrigger value="assignments" className="gap-1.5"><Puzzle className="h-3.5 w-3.5" />Assignments</TabsTrigger>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>Add-On Owner</TableHead>
                      ...
                      {filtered.map((a) => (
                        <TableRow key={a.id}>
                          ...
                          <TableCell>
                            {a.addonOwner ? (
                              <div>
                                <div className="text-sm font-medium">{a.addonOwner.name}</div>
                                <div className="text-xs text-muted-foreground">{a.addonOwner.designation}</div>
                              </div>
        ...
        <TabsContent value="audit" className="mt-4">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">Add-On Ownership Change History</CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {audit.length === 0 ? (
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Edit Add-On Owner — {editItem?.clientBusiness?.name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label>Add-On Owner</Label>
