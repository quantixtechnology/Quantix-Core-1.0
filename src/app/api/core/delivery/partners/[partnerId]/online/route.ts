// ============================================================================
// PATCH /api/core/delivery/partners/:partnerId/online
// Set a delivery partner's availability to ONLINE.
...
export const PATCH = withMiddleware({
  ...
  try {
    ...
    // DELIVERY_STAFF may only update their own record
    ...
    const updated = await db.deliveryPartner.update({
      where: { id: partnerId },
      data: { isOnline: true, availability: 'ONLINE' },
    });

    return NextResponse.json({ success: true, data: updated, message: 'Partner is now online' });
  } catch (error) {
