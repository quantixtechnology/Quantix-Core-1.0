...
export async function POST() {
  ...
  try {
    ...
    // =========================================================================
    // 2. PLATFORM PLANS (Standard + Pro — feature access only, no prices)
    // =========================================================================
    const plans = [
      {
        ...
        features: JSON.stringify(['Ecommerce Workflow', 'POS', 'Delivery Management', 'Custom Domain', 'White Label', 'Advanced Reports', 'API Access']),
      ...
      },
      {
        tier: 'PRO' as const,
        name: 'Quantix Pro',
        description: 'All workflows and advanced features for enterprise businesses',
        features: JSON.stringify(['All Standard Features', 'Pickup Workflow', 'Appointment Workflow', 'Subscription Workflow', 'Post-Service Workflow', 'Advanced Workflow Engine']),
    ...
    // Get the PRO plan for the demo subscription
    const proPlan = createdPlans.find(p => p.tier === 'PRO');
