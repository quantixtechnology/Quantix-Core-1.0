...
// POST /api/core/pos/billing — Calculate POS cart totals (auth required)
// ============================================================================
  ...
  try {
    ...
    for (const item of body.items) {
      if (!item.productId || !item.productName) {
        return NextResponse.json({ success: false, error: 'Each item must have productId and productName' }, { status: 400 });
      }
      if (!item.quantity || item.quantity <= 0) {
        return NextResponse.json({ success: false, error: `Invalid quantity for item: ${item.productName}` }, { status: 400 });
      }
      if (item.unitPrice === undefined || item.unitPrice < 0) {
        return NextResponse.json({ success: false, error: `Invalid unitPrice for item: ${item.productName}` }, { status: 400 });
      }
      if (item.mrp === undefined || item.mrp < 0) {
        return NextResponse.json({ success: false, error: `Invalid mrp for item: ${item.productName}` }, { status: 400 });
      }
    }
