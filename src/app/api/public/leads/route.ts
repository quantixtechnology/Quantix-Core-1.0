...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body._hp) {
      return Response.json({ success: true, data: { message: "Thanks! We'll contact you within 24 hours." } });
    }
    ...
    if (!name?.trim() || name.trim().length < 2) {
      return Response.json({ success: false, error: 'Please enter your full name' }, { status: 400 });
    }
    if (!businessName?.trim() || businessName.trim().length < 2) {
      return Response.json({ success: false, error: 'Please enter your business name' }, { status: 400 });
    }
    if (!phone) {
      return Response.json({ success: false, error: 'Please enter your phone number' }, { status: 400 });
    }
    const normalizedPhone = normalizePhone(phone);
    if (!/^[6-9]\d{9}$/.test(normalizedPhone)) {
      return Response.json({ success: false, error: 'Please enter a valid 10-digit mobile number' }, { status: 400 });
    }
    if (!businessType || !VALID_BUSINESS_TYPES.includes(businessType)) {
      return Response.json({ success: false, error: 'Please select a valid business type' }, { status: 400 });
    }
    if (!city?.trim() || city.trim().length < 2) {
      return Response.json({ success: false, error: 'Please enter your city' }, { status: 400 });
    }

    // ── Turnstile verification ─────────────────────────────────────────
    if (!captchaToken || typeof captchaToken !== 'string') {
      return Response.json({ success: false, error: 'CAPTCHA verification failed. Please try again.' }, { status: 400 });
    }
