...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // never happened.
    const stateGate = await guardStatusWrite({ orderId: order.id, businessId: biz.id, from: "PACKED", to: "IN_TRANSIT_TO_PROCESSING", allowInternal: true, custodyAction: true })
