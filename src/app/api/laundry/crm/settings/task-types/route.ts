// Task Types — fully configurable, tenant-scoped. No hardcoded task categories.
// Referenced by id on LaundryCrmTask.taskTypeId.
...
export async function GET(request: Request) {
  try {
    ...
    const rows = await prisma.laundryCrmTaskType.findMany({ where: where as never, orderBy: { displayOrder: "asc" } })
...
export async function POST(request: Request) {
  try {
    ...
    const max = await prisma.laundryCrmTaskType.aggregate({ where: { businessId: biz.id }, _max: { displayOrder: true } })
    const row = await prisma.laundryCrmTaskType.create({
      data: {
